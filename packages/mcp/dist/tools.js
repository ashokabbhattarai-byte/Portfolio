...
const scopeNote = (scope) => `Requires the "${scope}" scope on your API key. If the key lacks it the call fails with MISSING_SCOPE — ask the portfolio owner to grant it rather than retrying.`;
const blogFields = {
    title: z.string().min(2).max(200).describe('Article headline.'),
    excerpt: z
        .string()
        .max(400)
        .describe('One- or two-sentence summary used in listings and as the meta description fallback.'),
    content: z
        .string()
        .max(200_000)
        .describe('Article body in Markdown. Headings, lists, code fences and images are supported.'),
    slug: z
        .string()
        .regex(/^[a-z0-9]+(?:-[a-z0-9]+)*$/)
        .max(120)
...
export const tools = [
    ...
    },
    {
        ...
        description: 'Create a new article. It is ALWAYS saved as a draft and is not publicly visible, whatever else you send — publishing is a separate, separately-permissioned step. ' +
    ...
    },
    {
        ...
        schema: {
            id: z.string().max(100),
            ...Object.fromEntries(Object.entries(blogFields).map(([key, schema]) => [
                key,
                schema.optional(),
            ])),
            expectedVersion: z
                ...
                .min(1)
                .optional()
                .describe('Fails with VERSION_CONFLICT if the article changed since you read it.'),
        },
        run: (client, args) => {
    ...
    },
    {
        ...
        description: "Make an article publicly visible immediately. This is irreversible from the reader's point of view — the URL goes live and may be indexed. " +
    ...
    },
    {
        ...
        description: 'Schedule an article to publish automatically at a future time. The server publishes it even if nobody is signed in. ' +
        ...
        schema: {
            ...
            scheduledAt: z
                .string()
                .describe('ISO 8601 with offset, e.g. 2026-09-20T20:00:00+05:45 for Nepal time.'),
            timezone: z
                .string()
                .max(80)
                .optional()
    ...
    },
    {
        ...
        description: "Set an article's featured image or social (Open Graph) card from an image already in the media library. " +
    ...
    },
    {
        ...
        description: 'Download a publicly reachable image and store a copy in the media library, returning its id. ' +
    ...
    },
    {
        ...
        description: 'Generate an image with the configured AI image provider and store it in the media library. ' +
