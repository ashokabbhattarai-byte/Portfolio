              ...
              {data.topBlogs.length ? (
                data.topBlogs.map(({ blog, views }, index) => (
                  <Link
                    ...
                    key={blog.id}
                    href={`/admin/analytics/blogs/${blog.id}`}
                  >
              ...
              {data.topProjects.length ? (
                data.topProjects.map(({ project, views }, index) => (
                  <Link
                    className="analytics-ranking"
                    key={project.id}
                    href={`/admin/analytics/route?path=${encodeURIComponent(`/projects/${project.slug}`)}`}
                  >
                    <span>{String(index + 1).padStart(2, '0')}</span>
                  ...
                  <tbody>
                    {rows.map((row) => (
                      <tr key={row.path}>
                        <td>
                          <Link
                            href={`/admin/analytics/route?path=${encodeURIComponent(row.path)}`}
                          >
                            {row.path} ↗
                          </Link>
                        </td>
