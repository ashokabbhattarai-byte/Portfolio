...
export type Actor = {
  type: 'ADMIN' | 'AI_API_KEY' | 'SYSTEM';
  id: string;
  correlationId: string;
  apiKeyId?: string;
};
export type PublisherRequest = Request & {
  user?: AuthUser;
  publisher?: { id: string; scopes: string[] };
  correlationId?: string;
};
...
export function actorFrom(req: PublisherRequest): Actor {
  return {
    type: req.publisher ? 'AI_API_KEY' : 'ADMIN',
    id: req.publisher?.id ?? req.user?.id ?? 'unknown',
    apiKeyId: req.publisher?.id,
