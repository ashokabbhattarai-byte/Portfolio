...
export class ObjectStorageService implements ObjectStorage {
  ...
  constructor(
    ...
    private readonly legacy: StorageService,
  ) {
  ...
  private key(key: string) {
    if (!/^media\/[a-zA-Z0-9_-]+\.webp$/.test(key))
      fail('INVALID_OBJECT_KEY', 'Invalid media object key.');
  }
  getPublicUrl(key: string) {
    this.key(key);
    return this.provider === 'supabase'
      ? this.legacy.getPublicUrl(key)
      : `${this.config.getOrThrow<string>('MEDIA_PUBLIC_URL').replace(/\/$/, '')}/${key}`;
  }
  async upload(key: string, bytes: Buffer, mimeType: string) {
    this.key(key);
    if (!this.configured())
      fail('STORAGE_NOT_CONFIGURED', 'Media storage is not configured.', 503);
    if (this.provider === 'supabase')
      return (await this.legacy.upload(key, bytes, mimeType)).publicUrl;
    await this.s3!.send(
  ...
  async delete(key: string) {
    this.key(key);
    if (this.provider === 'supabase') return this.legacy.remove([key]);
    if (!this.s3)
      fail('STORAGE_NOT_CONFIGURED', 'Media storage is not configured.', 503);
    await this.s3.send(
